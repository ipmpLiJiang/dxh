 ...
 */
@RestController
@RequestMapping("/system/sb/bzyl")
public class SbBzylController {
    String message = "";
    @Autowired
    ISbBzylService sbBzylService;

    @GetMapping("/getXz")
    public FebsResponse getXz(String dtr) {
        ModelMap map = new ModelMap();
        int success = 0;
        List<SbBzyl> list = new ArrayList<>();
        try {
            Date bshdate = DateUtil.parse(dtr + "-01");
            Date ushdate = DateUtil.offset(bshdate, DateField.MONTH, -1);
            list = this.sbBzylService.getXzByInidnumberAndShDateLists(bshdate, ushdate,true);
            success = 1;
        } catch (Exception e) {
            message = "查询失败";
        }
    ...
    public FebsResponse getJs(String dtr) {
        ...
        List<SbBzyl> list = new ArrayList<>();
        try {
            list = this.sbBzylService.getJsByInidnumberAndShDateLists(bshdate, ushdate,true);
            success = 1;
        } catch (Exception e) {
            message = "查询失败";
        }
    ...
    public FebsResponse getBj(String dtr) {
        ...
        try {
            Date bshdate = DateUtil.parse(dtr + "-01");
            list = this.sbBzylService.getBjByShDateLists(bshdate);
            success = 1;
        } catch (Exception e) {
            message = "查询失败";
        }
